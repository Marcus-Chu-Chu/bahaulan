...
| export | `pipeline/export.py` | the mart tables | `exports/` CSVs, `dashboard.parquet`, `dashboard.hyper`, `metadata.json` | everything is written to `exports.tmp/` first and moved into place only after every file succeeds, so readers never see a half-updated folder |
| publish | `.github/workflows/pipeline.yml` | the files the export stage wrote | a commit on `main` and the four extracts on the `latest` release | the commit step is a no-op when nothing changed; a failed upload fails the job and the previous release assets stay in place |
...
commits changed snapshots, exports and log rows back to `main`, uploads the four extracts to
the `latest` release, and keeps a copy of `exports/` as a build artifact for 14 days. A
`concurrency` group named `pipeline` stops two runs from pushing at once.
